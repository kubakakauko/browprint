...
const getNonFunctionToStringLies = (x: string[]) =>
  !x
    ? x
    : x.filter((x) => !/object toString|toString incompatible proxy/.test(x))
        .length;
...
const getPluginLies = (plugins: PluginArray, mimeTypes: MimeTypeArray) => {
  ...
  pluginsList.forEach((plugin) => {
    const pluginMimeTypes = Object.values(plugin).map(
