...
!async function() {
	...
	const fingerprint = async () => {
		const timeStart = timer(0)
		const fingerprintTimeStart = timer(0)
		// @ts-ignore
		const [
			workerScopeComputed,
			voicesComputed,
		...
		const fingerprintTimeEnd = fingerprintTimeStart(0)
		...
		const hashTimeEnd = hashStartTime(0)
		const timeEnd = timeStart(0)
						...
						if (res.ok) {
							...
							<div class="flex-grid">
								<div class="col-six">
									...
									<div>hidden fingerprint: <span class="${TagGrade[tag] || ''}">${TagName[tag] || HTMLNote.SECRET
								}</span></div>
								</div>
								<div class="col-six">
									<div>org:</div>
		...
		fetch('https://creepjs-api.web.app/fp', {
			...
			`)
				...
				const computePoints = (x) => {
					return `<span class="scale-up grade-${x < 0 ? 'F' : x > 0 ? 'A' : ''}">${x > 0 ? `+${x}` : x < 0 ? `${x}` : ''
						}</span>`
				}
				...
				const styleChunks = (chunks) => chunks.map((y, yi) => {
					...
					return `<div>${y.map((x, xi) => `<span class="${x == '1' ? 'shadow' : 'blank'}" style="${x == 1 ? animate(xi + yi) : ''}"></span>`).join('')
						}</div>`
				}).join('')
				...
				<div class="visitor-info">
					<span class="time">fingerprints renewed <span class="${shouldStyle(renewedDateString) ? 'renewed' : ''}">${new Date(renewedDateString).toLocaleDateString()
					}</span></span>
					<div class="flex-grid relative">
						...
						<div class="col-six">
							...
							<div>visits: <span class="unblurred">${visits}</span> ${supervised > 0 ? `[<span>supervised</span>] ${computePoints(supervisedPointGain)}` : ''
					}</div>
					...
					})(persistence)}</span> ${traced > 0 ? `[<span>traced</span>] ${computePoints(tracedPointGain)}` : ''
					}</div>
							<div id="auto-delete">auto-delete in</div>
						...
						<div class="col-six">
							...
							<div>revisions (${'' + revisedKeys.length}): ${!revisedKeys.length ? 'none' : modal(
						`creep-revisions`,
						revisedKeys.join('<br>'),
